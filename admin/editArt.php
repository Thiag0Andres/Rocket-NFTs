...
$id = $_GET['id'];
$sql = "SELECT * FROM arte WHERE id = $id";
...
?>
    <div class="container">
        <h2 class="title-form">Editar Arte</h2>
        <form class="form-container" action="?pg=editArtdb" method="post">
            <input type="hidden" name="id" value="<?= $dados['id']; ?>">
            <label>Título da arte:</label>
            <input value="<?= $dados['title']; ?>" type="text" name="title" placeholder="Nome" />
            <label>Valor da arte:</label>
            <input value="<?= $dados['amount']; ?>" type="number" name="amount" />
            <label>Id do Artista:</label>
            <input value="<?= $dados['artistId']; ?>" type="number" name="artistId" />
